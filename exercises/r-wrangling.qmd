...
En règle générale, les formats *long* sont souvent préférables. C'est notamment
...
::: {.callout-tip}
2. Faire une jointure à gauche entre les données d'émissions et les données de cadrage. Comparer les émissions moyennes des villes sans *match* (celles dont des variables bien choisies de la table de droite sont `NA`) avec celles où on a bien une valeur correspondante dans la base Insee
...
ggplot(emissions_merged) +
  geom_histogram(aes(x = empreinte, y = ..density..)) +
  scale_x_log10()
```
