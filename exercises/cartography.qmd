...
Créer des ronds proportionnels

```{r}
mf_map(population_dep)
mf_map(population_dep, var = "densite", type = "prop")
```
...
```{r}
mf_map(population_dep)
mf_map(population_dep, var = c("densite", "densite"), type = "prop_choro", breaks = 4)
mf_layout(
  title = "Population en France"
)
```

```{r}
idf <- population_dep %>% filter(INSEE_REG == 11) 
```


```{r}
mf_map(idf)
mf_map(idf, var = c("densite", "densite"), type = "prop_choro")
mf_layout(
    title = "Population en Île de France"
)
```
